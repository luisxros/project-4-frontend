...
function SongPage() {
  ...
  useEffect(() => {
    ...
    function getSongs() {
      fetch('https://tune-it-up-frontend.herokuapp.com/songs')
      .then(res => res.json())
    ...
    function handleDelete(deletedSong) {
      fetch(`https://tune-it-up-frontend.herokuapp.com/songs/${deletedSong.id}`, {
        method: 'DELETE',
      })
      .then(() => {
        getSongs()
    ...
    function handleUpdate(event, formInputs) {
      event.preventDefault();
      fetch(`https://tune-it-up-frontend.herokuapp.com/songs/${formInputs.id}`, {
        method: 'PUT',
        body: JSON.stringify(formInputs),
        headers: {
          'Content-Type': 'Application/json'
